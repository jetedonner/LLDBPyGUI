...
int main() {
    ...
    // Load the dynamic library
    void *handle2 = dlopen("./libexternal2.dylib", RTLD_LAZY);
    if (!handle2) {
        fprintf(stderr, "❌ Failed to load library (2): %s\n", dlerror());
        return 1;
    }

    // Get the function pointer
    void (*callExternalFunc2)() = dlsym(handle2, "callExternalFunc2");
    if (!callExternalFunc2) {
        fprintf(stderr, "❌ Failed to find symbol: %s\n", dlerror());
        dlclose(handle2);
        return 1;
    }
    ...
    while (1) {
        ...
        if (idx % 4 == 0) {
            callExternalFunc2();  // Call the function from the dylib
        }
        if (idx % 9 == 0) {
