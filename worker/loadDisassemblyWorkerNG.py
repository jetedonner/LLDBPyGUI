...
class LoadDisassemblyWorkerNG(BaseWorker):
	...
	def __init__(self, driver, modulePath, initTable = True, parent=None):
		super(LoadDisassemblyWorkerNG, self).__init__(driver)
		self.modulePath = modulePath
		self.initTable = initTable
		self.signals = LoadDisassemblyWorkerNGSignals(parent)
	...
	def disassemble_entire_target(self, target):
		...
		for module in self.target.module_iter():
			if module.GetFileSpec().fullpath != self.modulePath:
				print(f"LoadDisassemblyWorkerNG.disassemble_entire_target() => module.GetFileSpec().fullpath: {module.GetFileSpec().fullpath} / {self.modulePath}....")
				continue
			else:
				# if idxOuter != 0:
				# 	idxOuter += 1
				...
				for sym in module:
					...
					self.signals.loadSymbolCallback.emit(sym.GetName())
