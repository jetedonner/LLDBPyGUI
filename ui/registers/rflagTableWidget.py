...
from config import *
from helper.variableHelper import is_int
...
class RFlagTableWidget(BaseTableWidget):
	...
	def addRow(self, flag, value, txt, desc="", bold=False):
		currRowCount = self.rowCount()
		self.setRowCount(currRowCount + 1)
		self.addItem(currRowCount, 0, str(flag), desc, bold)
		self.addItem(currRowCount, 1, str(value), desc, bold)
		self.addItem(currRowCount, 2, str(txt), desc, bold)
		# if self.cellWidget(currRowCount, 0) != None:
		# 	self.cellWidget(currRowCount, 0).setToolTip(desc)
		self.setRowHeight(currRowCount, 18)

	def addItem(self, row, col, txt, desc="", bold=False):
	...
	def loadRFlags(self, debugger):
		...
		for reg in GPRs:
			...
			if reg.name == "cpsr":
				...
				break
		# if not rflags_reg:
		# 	result.AppendError("Could not find rflags registers.")
		# 	return

		print(f"self.rflags_value: {self.rflags_value} / is_int(self.rflags_value): {is_int(self.rflags_value)} ...")
		if True and is_int(self.rflags_value):

			# is_int(self.rflags_value)
			cpsr_value = int(self.rflags_value)
			...
			self.addRow("FLAG OVERVIEW:", "", "", "", True)
			self.addRow("--- DATA TYPE ---", "--- VALUE ---", "--- INFOS ---", "", True)
		...
		self.addRow("FLAG DETAILS:", "", "", "", True)
		self.addRow("--- FLAG NAME ---", "--- OPTION / VALUE ---", "--- DESCRIPTION ---", "", True)
		self.decode_cpsr_flags(frame)
		self.addRow("", "", "")
		print(f"========================")
		cpsr_value = int(self.rflags_value, 16)
		self.decode_cpsr(cpsr_value)
		print(f"========================")
		# self.wdtLabel.setText(
		# 	"rFlags / eFlags: ")  # + hex(self.tblRFlag.rflags_value) + " / " + format(self.tblRFlag.rflags_value, 'b') + " / " + pfl_cmd(get_main_window().driver.debugger, "", res, []))
		self.addRow("FLAG OVERVIEW:", "", "", "", True)
		...
		self.addRow("octal", str(oct(cpsr_value)), "Complete Flag with all bits as OCTAL")
		self.addRow("binary", format(cpsr_value, 'b'), "Complete Flag with all bits as BINARY")
		quick_decoded = self.decode_cpsr(cpsr_value)
		self.addRow("quick", quick_decoded,
					"Quickview all flags")  # pfl_cmd(get_main_window().driver.debugger, "", res, []), "Quickview all flags")
		# logDbgC(
		...
		self.addRow("", "", "")
		self.addRow("FLAG DETAILS:", "", "", "", True)
		self.addRow("--- FLAG NAME ---", "--- OPTION ---", "--- DESCRIPTION ---", "", True)
		...
		if (cpsr_value >> 4) & 1:
			...
			self.addRow("AF", "Auxiliary Carry",
						"=1 > AC (Auxiliary Carry) / =0 > NA (No Auxiliary Carry) - Mask: 0x0010")
		else:
		...
		if (cpsr_value >> 9) & 1:
			...
			self.addRow("IF", "Interrupt Enable",
						"=1 > EI (Enable Interrupt) / =0 > DI (Disable Interrupt) - Mask: 0x0200")
		else:
		...
		if (cpsr_value >> 16) & 1: self.addRow("RF", "Resume Flag",
											   "Resume flag (386+ only) - Mask: 0x0001 0000")  # flags.append("RF (Resume Flag)")

		if (cpsr_value >> 17) & 1: self.addRow("VM", "Virtual-8086 Mode",
											   "Virtual 8086 mode flag (386+ only) - Mask: 0x0002 0000")  # flags.append("VM (Virtual-8086 Mode)")

		if (cpsr_value >> 18) & 1: self.addRow("AC", "Alignment Check",
											   "Alignment Check (486+, ring 3) SMAP Access Check (Broadwell+, ring 0-2) - Mask: 0x0004 0000")  # flags.append("AC (Alignment Check)")

		if (cpsr_value >> 19) & 1: self.addRow("VIF", "Virtual Interrupt",
											   "Virtual interrupt flag (Pentium+) - Mask: 0x0008 0000")  # flags.append("VIF (Virtual Interrupt)")

		if (cpsr_value >> 20) & 1: self.addRow("VIP", "Virtual Interrupt Pending",
											   "Virtual interrupt pending (Pentium+) - Mask: 0x0010 0000")  # flags.append("VIP (Virtual Interrupt Pending)")

		if (cpsr_value >> 21) & 1: self.addRow("ID", "ID Flag",
											   "Able to use CPUID instruction (Pentium+) - Mask: 0x0020 0000")  # flags.append("ID (ID Flag)")

	# import lldb

	def addDecodedFlag(self, cpsr_value, flag, shift, setVal, notSetVal, desc):
		self.addRow(flag,
					f"{setVal if (cpsr_value >> shift) & 1 else notSetVal} ({'✅ Set' if (cpsr_value >> shift) & 1 else '❌ Not Set'})",
					desc)

	def decode_cpsr_flags(self, frame):
		"""
        Decodes the ARM CPSR registers and prints the state of the flags.
        """
		cpsr_reg = frame.FindRegister('cpsr')
		if not cpsr_reg.IsValid():
			print("CPSR registers not found.")
			return

		cpsr_value = cpsr_reg.unsigned
		print(f"CPSR registers value: 0x{cpsr_value:08x}")

		print("Condition Code Flags:")
		print(f"  N (Negative): {'Set' if (cpsr_value >> 31) & 1 else 'Not Set'}")
		# self.addRow(f"{'NG' if (cpsr_value >> 31) & 1 else 'PL'}",f"N (Negative)", f"{'Set' if (cpsr_value >> 31) & 1 else 'Not Set'}", f"1 == NG (Negative) / 0 == PL (Positive) - Mask: 0x0080")
		self.addDecodedFlag(cpsr_value, "N (Negative)", 31, 'NG', 'PL',
							"1 == NG (Negative) / 0 == PL (Positive) - Mask: 0x0080")
		print(f"  Z (Zero):     {'Set' if (cpsr_value >> 30) & 1 else 'Not Set'}")
		self.addDecodedFlag(cpsr_value, "Z (Zero)", 30, 'ZR', 'NZ',
							"1 == ZR (Zero) / 0 == NZ (Not Zero) - Mask: 0x0040")
		print(f"  C (Carry):    {'Set' if (cpsr_value >> 29) & 1 else 'Not Set'}")
		# =1 > CY(Carry) / = 0 > NC(No Carry) - Mask: 0x0001
		self.addDecodedFlag(cpsr_value, "C (Carry)", 29, 'CY', 'NC',
							"1 == CY (Carry) / 0 == NC (No Carry) - Mask: 0x0001")
		print(f"  V (Overflow): {'Set' if (cpsr_value >> 28) & 1 else 'Not Set'}")
		# =1 > OV(Overflow) / = 0 > NV(Not Overflow) - Mask: 0x0800
		self.addDecodedFlag(cpsr_value, "V (Overflow)", 28, 'OV', 'NV',
							"1 == OV (Overflow) / 0 == NV (Not Overflow) - Mask: 0x0800")
		print(f"  Q (Saturation): {'Set' if (cpsr_value >> 27) & 1 else 'Not Set'}")
		self.addDecodedFlag(cpsr_value, "Q (Saturation)", 27, 'Q', 'NQ',
							"1 == Q (Saturated) / 0 == NQ (Not Saturated) - Mask: 0xXXXX")
