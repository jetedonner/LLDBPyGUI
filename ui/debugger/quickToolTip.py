...
class QuickToolTip:
	...
	def get_memory_addressAndOperands(self, debugger, operands):
		logDbgC(f"get_memory_addressAndOperands() => operands: {operands} ...")
	...
	def extractOperand(self, string):
		...
		registers = re.findall(r'\bx\d+\b', string)
		print(registers)  # Output: ['x10', 'x9']
		if len(registers) > 0:
			return registers[0]
		else:
	...
	def splitOperands(self, openrands):
		...
		# if "[" in openrands and "]" in openrands:
		# 	parts = re.split(r',\s*', openrands.replace('[', '').replace(']', ''))
		# 	logDbgC(f"parts: {parts} ...")
		# 	return parts
		# else:
	...
	def getOperandsText(self, openrands):
		...
		if SizeDirPtrs.startswith(part1):
			...
			# logDbgC(f"getOperandsText({openrands}) => ELSE!!! ... ")
			# pass
			arrOperands = openrands.split(", ")
		# logDbgC(f"arrOperands == {arrOperands} ... ")
	...
	def getOperandsArr(self, openrands):
		operandsText = ""
		...
		if "[" in openrands and "]" in openrands:
			arrOperands = re.split(r',\s*', openrands.replace('[', '').replace(']', ''))
			logDbgC(f"parts: {arrOperands} ...")
			return arrOperands
		else:
			part1, part2 = self.splitOperands(openrands)
			#		print(f"getOperandsText->part1: {part1}")
	...
	def sanitizeOperandsPart(self, part):
		return part.replace("0x", "").replace("]", "").replace("\!", "").replace(f"!", f"")
	...
	def getQuickToolTip(self, openrands, debugger):
		...
		arrOperands = self.getOperandsArr(openrands)
		arrOperandsAlready = []
		for operand in arrOperands:
			if not operand.startswith(COMMENT_CHAR) and not operand in arrOperandsAlready:
				arrOperandsAlready.append(operand)
				rbp_value = frame.EvaluateExpression(f"${operand}").GetValueAsUnsigned()
				if tooltip != "":
					tooltip += f'\n----------------------\n'
				tooltip += f"Operand:\t{operand}:\n- Address:\t{hex(rbp_value)} ({rbp_value})"
				...
				if isMinus:
					try:
						offset_value = int(self.sanitizeOperandsPart(parts[1]), 16) # int(parts[1].replace("0x", "").replace("]", "").replace("\!", "").replace(f"!", f""), 16)
						address = int(rbp_value - offset_value)
					except Exception as e:
						print(f"Error in getQuickToolTipNG (isMinus): {e}")
						address = 0
				elif isSolo:
					try:
						address = int(rbp_value)
					except Exception as e:
						print(f"Error in getQuickToolTipNG (isSolo): {e}")
						address = 0
				else:
					try:
						offset_value = int(self.sanitizeOperandsPart(parts[1]), 16) # int(parts[1].replace("0x", "").replace("]", "").replace("\!", "").replace(f"!", f""), 16)
						...
						print(f"Error in getQuickToolTipNG (else): {e}")
						address = 0
