...
from helper.watchpointHelper import WatchpointHelper
from lib.fileInfos import WatchpointValueKindString
from lib.settings import *
from ui.base.baseTableWidget import BaseTableWidget
from ui.debugger.assemblyTableWidget import DisassemblyImageTableWidgetItem
...
#		self.addRow(True, 1, "0x10000", "noname", 0, "nocond")
	...
	def on_double_click(self, row, col):
		if col == 0:
			...
			self.window().handle_showMemory(self.item(row, 2).text())
		elif col == 5:
			try:
				memAddr = int(self.item(row, 5).text(), 16)
				self.window().handle_showMemory(memAddr) # .wdtDisassembly.tblDisassembly.viewAddress(memAddr)
...
#			self.txtMultiline.table.setBPAtAddress(loadAddr, True, False)
		...
		self.addRow(wp.IsEnabled(), wp.GetID(), hex(wp.GetWatchAddress()), hex(wp.GetWatchSize()), WatchpointValueKindString(wp.GetWatchValueKind()), wp.GetWatchSpec(), str(("r" if wp.IsWatchingReads() else "") + ("w" if wp.IsWatchingWrites() else "")), wp.GetHitCount(), wp.GetIgnoreCount(), wp.GetCondition())
...
#				self.item(i, 0).enableBP(state)
				...
				self.cellWidget(i, 6).setCurrentText(str(name))
...
#		item.setIcon(ConfigClass.iconEyeRed)
		...
		combo = QComboBox()
		combo.addItems(["r", "w", "rw"])
		combo.setCurrentText(str(name))
		self.setCellWidget(currRowCount, 6, combo)
