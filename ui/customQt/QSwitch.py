...
class SwitchPrivate(QObject):
	...
	def draw(self, painter):
		...
		painter.setBrush(self.mGradient)
		painter.drawRoundedRect(r.adjusted(int(margin), int(margin), int(-margin), int(-margin)), r.height() / 2,
								r.height() / 2)
...
class Switch(QAbstractButton):
	...
	def setSwitchSize(self, switchSize: SwitchSize = SwitchSize.Small):
		...
		if self.switchSize == SwitchSize.ExtraSmall:
			fixedSize = QSize(30, 16)
		elif self.switchSize == SwitchSize.Small:
...
class QSwitch(QWidget):
	...
	def __init__(self, descTxt: str, switchSize: SwitchSize = SwitchSize.Small,
				 switchLabelPos: SwitchLabelPos = SwitchLabelPos.Trailing, lblToggleSwitch=True, parent=None):
