...
def find_main(debugger, mainFunctionName="main"):
	...
	if main_symbol.GetSize() == 0:
		logDbgC(f"Could not find '{mainFunctionName}' function ...")
		return 0, None
...
def GetFileHeader(exe):
	...
	for header in macho.headers:
		print("CPU Type:", header.header.cputype)
		print("File Type:", header.header.filetype)
		# print("Filename:", header.header.filename)
...
class FileInfos():
	...
	@staticmethod
	def get_target_filesize(target):
		...
		file_spec = main_module.GetFileSpec()
	...
	#
	#     table.addRow("Magic", MachoMagic.to_str(MachoMagic.create_magic_value(mach_header.magic)),
	#                  hex(mach_header.magic))
	#     table.addRow("CPU Type", MachoCPUType.to_str(MachoCPUType.create_cputype_value(mach_header.cputype)),
	#                  hex(mach_header.cputype))
	#     table.addRow("CPU SubType", str(mach_header.cpusubtype), hex(mach_header.cpusubtype))
	#     table.addRow("File Type", MachoFileType.to_str(MachoFileType.create_filetype_value(mach_header.filetype)),
	...
	#     table.addRow("Flags", MachoFlag.to_str(MachoFlag.create_flag_value(mach_header.flags)),
	#                  hex(mach_header.flags))
	#
	#     table.addRow("----", str("-----"), '-----')
