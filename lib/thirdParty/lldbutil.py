...
def int_to_bytearray(val, bytesize, byteOrder='big'):
	"""Convert an integer to a bytearray of given size in little-endian format."""
	if bytesize < 1:
		raise ValueError("Byte size must be at least 1")

	# Ensure the value fits in the given number of bytes
	max_val = (1 << (bytesize * 8)) - 1
	min_val = -(1 << (bytesize * 8 - 1))
	if not (min_val <= val <= max_val):
		# raise OverflowError(f"Value {val} does not fit in {bytesize} bytes")
		return None
	# Convert using int.to_bytes
	# return val.to_bytes(bytesize, byteorder='little', signed=(val < 0))
	sRet = val.to_bytes(bytesize, byteorder=byteOrder, signed=(val < 0))
	return sRet
...
#     unpacked = struct.unpack(fmt, bytes)
#     return unpacked[0]

def bytearray_to_int(bytes, bytesize):
	"""Utility function to convert a bytearray into an integer.

	It interprets the bytearray in the big endian format.
	"""
	import struct
		...
		if not function:
			...
			print("  frame #{num}: {addr:#016x} {mod}`{func} at {file}:{line} {args}".format(
				num=i, addr=load_addr, mod=mods[i], func='%s [inlined]' %
														 funcs[i] if frame.IsInlined() else funcs[i], file=files[i],
				line=lines[i], args=get_args_as_string(
...
def print_registers(frame, string_buffer=False):
	"""Prints all the registers sets of the frame."""
	...
	print("Frame registers (size of registers set = %d):" % registerSet.GetSize(
	), file=output)
