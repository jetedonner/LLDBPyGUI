...
class MachO:
	...
	def parse_all_load_commands(self):
		...
		for _ in range(ncmds):
			...
			if (cmd == LOAD_COMMAND_TYPES["LC_SEGMENT"] or
				...
				segment_dict = {
					"segname": segname,
					"vaddr": vaddr & 0xFFFFFFFFFFFFFFFF if is_64_bit else vaddr & 0xFFFFFFFF,
					# Virtual address - ensure unsigned
					"vsize": vsize & 0xFFFFFFFFFFFFFFFF if is_64_bit else vsize & 0xFFFFFFFF,
					# Virtual size - ensure unsigned
					"offset": offset & 0xFFFFFFFFFFFFFFFF if is_64_bit else offset & 0xFFFFFFFF,
					# File offset - ensure unsigned
					"size": size & 0xFFFFFFFFFFFFFFFF if is_64_bit else size & 0xFFFFFFFF,
					# File size - ensure unsigned
					...
					# uint32_t - protection flags, ensure unsigned
			...
			# Process dylib commands (LC_LOAD_DYLIB, LC_ID_DYLIB, LC_LOAD_WEAK_DYLIB, LC_REEXPORT_DYLIB)
				...
				dylib_dict = {
					...
					# uint32_t - version number, ensure unsigned
					"dylib_compat_version": dylib_compat_version & 0xFFFFFFFF,
					# uint32_t - compatibility version, ensure unsigned
			...
			# Process LC_UNIXTHREAD (alternative entry point for older samples)
				...
				if len(thread_data) >= 8:  # Need at least flavor and count
					...
					if cputype == CPU_TYPE_X86_64 and len(thread_data) >= 136:
						...
						pc_offset = 8 + 29 * 8 + 8 + 8  # Skip to PC registers
				...
				entry_point = {
					...
					# uint64_t - entry address, ensure unsigned
				...
				if blob_offset + 8 <= len(signature_data):
					...
					if blob_type >= CSSLOT_SIGNATURESLOT:
						...
						if blob_magic == CSMAGIC_BLOBWRAPPER:
							cert_list = self._parse_certificate_blob(signature_data, blob_offset, blob_length,
																	 cert_index)
							if cert_list:
					...
					# Parse launch constraint blobs (slots 8-11)
						...
						if blob_magic == CSMAGIC_EMBEDDED_LAUNCH_CONSTRAINT:
							constraint_info = self._parse_launch_constraint_blob(signature_data, blob_offset,
																				 blob_length, blob_type)
	...
	def _parse_code_directory_blob(self, signature_data, offset, length):
		...
		try:
			...
			print(f"signature_data: {signature_data} / cd_start: {cd_start} / ident_offset: {ident_offset} ...")
			...
			if ident_offset > 0 and cd_start + ident_offset < len(signature_data):
				...
				ident_start = cd_start + ident_offset - 0x8
				ident_end = signature_data.find(b'\x00', ident_start)
				print(f"ident_start: {ident_start} / ident_end: {ident_end} ...")
...
def main():
	...
	def get_formatted_header(arch=None):
		...
		if isinstance(raw_header, dict) and 'magic' in raw_header:
			...
			if arch:
				...
				macho_instance = macho.macho if hasattr(macho, 'macho') else list(macho.architectures.values())[
					0] if macho.is_fat else macho
			if macho_instance:
	...
	print_section_for_arch("Code Signature", lambda arch: get_arch_data('code_signature_info'), args.all,
						   args.signature)
